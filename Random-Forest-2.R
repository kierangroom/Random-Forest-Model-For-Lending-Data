...
model <- randomForest(as.factor(revol_util) ~ loan_amnt + int_rate + emp_length + home_ownership + annual_inc + zip_code + dti + open_acc + pub_rec + total_rec_late_fee + acc_now_delinq, data = train)
vip(model)
#model
vi(model)
